...
function Tasks() {
  ...
  useEffect(() => {
    if (debouncedValue) {
      requestHandler.get(
        `/api/work_numbers?search=${debouncedValue}`,
        setWorkNumbers,
        null,
        null
      );
  ...
  function fetchHandlers() {
    if (Object.keys(editTask).length > 0)
      requestHandler.get(
        `/api/admin_department_handlers/${editTask.department?.id}`,
        setHandlers
      );
  }
    ...
    <SideNav>
      <div>
        <button
          ...
          className='relative rounded bg-gray-400 hover:bg-gray-300 right-12 w-[5rem] p-2 mb-15 float-right mb-5 '
        >
          Export
        </button>
            ...
            {(Array.isArray(tasks.data) ? tasks.data : []).map(
              (task, index) => {
                return (
                  <tr
                    key={task.id || index}
                    className='overflow-auto h-[30px] bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600 py-1'
                  >
                    <th
                      scope='row'
                    ...
                    <th
                      scope='row'
                      title={
                        (task.task_type && task.task_type.description) || ''
                      }
              ...
              <form className='max-w-md mx-auto h-auto p-3 overflow-y-scroll'>
                ...
                <div className='relative z-0 w-full mb-5 group'>
                  ...
                  >
                    <option
                      value=''
                      className='bg-transparent text-gray-900 dark:text-red-300'
                    >
                      Select Task Type *
                    </option>
                    {(Array.isArray(taskTypes) ? taskTypes : [red]).map(
                      (type, index) => {
                        'block py-2.5 px-0 w-full text-sm border-0 bg-transparent border-b-2 border-gray-300 appearance-none dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer';
                        return (
                          <option
                            key={type.id || index}
                            value={type.id}
                ...
                <div className='relative z-0 w-full mb-5 group'>
                  ...
                  <hr className='w-full border-[1px] border-gray-300' />
                  {(errors.admin_handler_id ||
                    errors.errors?.admin_handler_id) && (
                    <p className='text-red-500 my-2 py-1'>
                ...
                <div className='relative z-0 w-full mb-5 group'>
                  ...
                  >
                    ...
                    {(Array.isArray(handlers.departmentHeads)
                      ? handlers.departmentHeads
                      : []
                    ).map((type, index) => {
                      return (
                        <option
                  ...
                  <hr className='w-full border-[1px] border-gray-300' />
                  {(errors.department_handler_id ||
                    errors.errors?.department_handler_id) && (
                    <p className='text-red-500 my-2 py-1'>
                ...
                <div className='my-10'>
                  <ReactSelect
                    ...
                    name='work_number'
                    placeholder={
                      editTask.client?.employee_id || 'Enter Work Number'
                    }
                    options={(Array.isArray(workNumbers.data)
                      ? workNumbers.data
                      : []
                    ).map(item => {
                      return {
