...
function Id({ user }) {
  ...
  function submitMessage(e) {
    e.preventDefault();
    if (newMessage) {
      requestHandler.post('/api/messages', {
        message: newMessage,
        taskId: taskId,
      });
      setNewMessage('');
  ...
  function scrollToLastMessage() {
    if (messagesEnd.current) {
      ...
      if (
        messagesEnd.current.getBoundingClientRect().top >
        chatContainer.current.getBoundingClientRect().bottom
      ) {
  ...
  return (
    <SideNav>
      <div className='w-full'>
        <h1 className='font-bold text-xl text-lg text-center pl-3 mb-10'>
          {currentTask.task?.name}
        </h1>
        <div className='flex float-right'>
          <span className='mr-1'>{currentTask.task?.from_date ?? ''}</span> -{' '}
          <span className='ml-1'>{currentTask.task?.from_date ?? ''}</span>
        </div>
        <div className='mb-5 '>
          <h1 className='font-normal text-lg'>Handlers</h1>
          <ol className='border rounded w-[40vw]'>
            {(Array.isArray(currentTask.handler)
              ? currentTask.handler
              : []
            ).map((taskHandler, ind) => {
              return (
                <li
                  key={taskHandler.id || ind}
                  className='font-normal text-lg pl-3'
                >
                  - {taskHandler.name ?? ''}
                </li>
              );
      ...
      <div>
        ...
        <div className='flex'>
          ...
          <h1 className='font-normal text-lg text-center pl-3'>
            {currentTask.admin?.name ?? 'N/A'}
          </h1>
        </div>
        <div className='flex'>
          <h1 className='italic text-lg'>Department Head:</h1>
          <h1 className='font-normal text-lg text-center pl-3'>
            {currentTask.department_head?.name ?? 'N/A'}
          </h1>
        </div>
      </div>
      ...
      <div className='w-full'>
        <div
          className='w-full rounded-md bg-white dark:bg-gray-900 h-[calc(100vh-200px)] overflow-y-scroll'
          ref={chatContainer}
        >
        ...
        {currentTask.can_edit ? (
          ...
          <h1 className='text-lg text-gray-400 text-center'>
            Cannot reply in this conversation
          </h1>
        )}
      </div>
    </SideNav>
